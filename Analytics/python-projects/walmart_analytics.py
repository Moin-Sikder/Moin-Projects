...
class MartAnalytics:
    ...
    def project_4_marketing_mix_modeling(self):
        """Project 4: Marketing Mix Modeling - FIXED VERSION"""
        print("\n" + "="*50)
        ...
        # Impact of external factors on sales - FIXED: Use proper subplot indexing
        factors = ['Temperature', 'Fuel_Price', 'CPI', 'Unemployment']
        
        # Plot each factor in its own subplot
        ...
        X_mmm = self.df[['Marketing_Spend_Online', 'Markdown1', 'Markdown2', 
                         'Temperature', 'Fuel_Price', 'CPI', 'Unemployment', 'Holiday_Flag']].copy()
        
        # Handle Holiday_Flag encoding safely
        ...
        X_mmm = X_mmm.fillna(0)
        y_mmm = y_mmm.fillna(0)
        ...
        # Calculate R-squared
        r2_score = model.score(X_mmm, y_mmm)
        
        print("\n🔍 MARKETING MIX INSIGHTS:")
        print("Factor Impact on Sales (sorted by absolute impact):")
        print(coefficients.round(4))
        print(f"\nModel R-squared: {r2_score:.4f}")
        
        # Create a separate visualization for factor impacts
        plt.figure(figsize=(10, 6))
        impacts = coefficients.sort_values('Coefficient', ascending=False)
        colors = ['green' if x > 0 else 'red' for x in impacts['Coefficient']]
        plt.barh(impacts['Feature'], impacts['Coefficient'], color=colors)
        plt.xlabel('Coefficient Value')
        plt.title('Marketing Mix Factors Impact on Sales\n(Green = Positive, Red = Negative)', fontweight='bold')
        plt.tight_layout()
        plt.show()
